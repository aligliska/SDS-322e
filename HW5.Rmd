...
glimpse(penguins)
glimpse(penguins_2007)
```
*Compared to the original `penguins` dataset that has 344 observations, the new `penguins_2007` dataset only has 110 observations. The number of observations in `penguins_2007` is approximately 32% of the number of observations in `penguins`.*
...
```{R}
penguins_2007 %>% filter(bill_length_mm > 45) %>% filter(bill_length_mm < 55)
```
...
```
*Yes, there are cases where the ratio is greater than 3.5. The Gentoo species of penguins shows a ratio of `bill_length_mm` to `bill_depth_mm` that exceeds 3.5.*
...
```{R}
penguins_2007 %>% arrange(bill_length_mm) %>% group_by(species) %>%
  slice(1:3) 
penguins_2007 %>% arrange(bill_length_mm) %>% group_by(species) %>%
  slice(1:3) %>% count(sex=="female")
```
*7 of the 9 penguins (3 Chinstrap, 3 Gentoo, 1 Adelie) that had the shortest beaks were female. The other two penguins [Adelie] consisted of a male and an unidentified penguin sex.*
...
```{R}
penguins_2007 %>% group_by(species) %>% summarise(mean(bill_ratio, na.rm=T))
penguins_2007 %>% group_by(species) %>% summarise(sd(bill_ratio, na.rm = T))
```
*The Gentoo species has the greatest average bill ratio.*
...
```{R}
penguins_2007 %>% group_by(species, island) %>% summarise(n())
```
*The Adelie species appears on all three islands.*
...
```{R warning=F}
penguins_2007 %>% ggplot(aes(x=body_mass_g, fill = sex)) + geom_density() + facet_grid(species~.)
```
*The Adelie species has the most NAs for `sex`. The Chinstrap species shows the least sexual dimorphism.*
...
```{R}
penguins_2007 %>% ggplot(aes(x=flipper_length_mm, y=bill_ratio)) + geom_point() + geom_smooth(method = "lm")
```
*There appears to be a postive relationship between flipper length and bill ratio.*
...
```{R}
penguins_2007 %>% ggplot(aes(x=flipper_length_mm, y=bill_ratio, color=species)) + geom_point() + geom_smooth(method = "lm")
```
*When considering each species separately, it does not appear that there is a positive relationship between flipper length and bill ratio (i.e. actually more of no correlation/slightly negative). The apparant postive relationship was resulted from to inherent differences in species characteristics.*
...
```{R}
penguins %>% ggplot(aes(x=body_mass_g, y=flipper_length_mm, color=species, shape=island)) + geom_point() + facet_wrap(~year) +
  scale_x_continuous(n.breaks = 3)
```
*There appears to be a relationship between body mass and flipper length, across both the overall distribution of penguins and within each species. Over the span of three years, the distribution of flipper lengths for species became more overlapped.*
